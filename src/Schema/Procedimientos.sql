...
BEGIN
	IF OPCION=1 THEN
		INSERT INTO CLIENTE VALUES(R_INS,N_INS,A_INS,T_INS,1);
	ELSIF OPCION=2 THEN
		UPDATE CLIENTE SET NOMBRE=N_INS,APELLIDO=A_INS, TELEFONO=T_INS, ESTADO=1 
		WHERE RUT=R_INS;
	ELSIF OPCION=3 THEN
		UPDATE CLIENTE SET ESTADO=0 WHERE RUT=R_INS;
	ELSIF OPCION=4 THEN
		UPDATE CLIENTE SET ESTADO=1 WHERE RUT=R_INS;
	END IF;
END;
/
...
A_INS VARCHAR, T_INS VARCHAR, ND_INS VARCHAR, S_INS NUMBER) AS
	BEGIN
	IF OPCION=1 THEN
		INSERT INTO JEFEDEPTO VALUES(R_INS,N_INS,A_INS,T_INS,1,ND_INS,S_INS);
	ELSIF OPCION=2 THEN
		UPDATE JEFEDEPTO SET NOMBRE=N_INS,APELLIDO=A_INS, TELEFONO=T_INS, 
		NOMBREDEPTO=ND_INS, SUELDO=S_INS, ESTADO=1 
		WHERE RUT=R_INS;
	ELSIF OPCION=3 THEN
		UPDATE JEFEDEPTO SET ESTADO=0 WHERE RUT=R_INS;
	ELSIF OPCION=4 THEN
		UPDATE JEFEDEPTO SET ESTADO=1 WHERE RUT=R_INS;
	END IF;
	END;
/
----------------------------------------------------------------------------------------------
...
BEGIN
	IF OPCION=1 THEN
		INSERT INTO VENDEDOR SELECT R_INS,N_INS,A_INS,T_INS,1,FI_INS,SB_INS,REF(JD) 
		FROM JEFEDEPTO JD WHERE JD.RUT=JEFE_RUT;	
	ELSIF OPCION=2 THEN
		UPDATE VENDEDOR SET NOMBRE=N_INS,APELLIDO=A_INS, TELEFONO=T_INS, 
		FECHA_INGRESO=FI_INS, SUELDO_BASE=SB_INS, ESTADO=1, 
		JEFE = (SELECT REF(JD) FROM JEFEDEPTO JD WHERE JD.RUT=JEFE_RUT) 
		WHERE RUT=R_INS;
	ELSIF OPCION=3 THEN
		UPDATE VENDEDOR SET ESTADO=0 WHERE RUT=R_INS;
	ELSIF OPCION=4 THEN
		UPDATE VENDEDOR SET ESTADO=1 WHERE RUT=R_INS;
	END IF;
END;
/
...
BEGIN
	...
	WHERE C.RUT=RUT_CLIENTE AND V.RUT=RUT_VENDEDOR ;	
END;
/
----------------------------------------------------------------------------------------------
----------------------------------------CRUD PRODUCTO-----------------------------------------
...
P_INS NUMBER, S_INS NUMBER, SC_INS NUMBER) AS
	BEGIN
	IF OPCION=1 THEN
		INSERT INTO PRODUCTO VALUES(C_INS,D_INS,P_INS,S_INS,SC_INS,1);
	ELSIF OPCION=2 THEN
		UPDATE PRODUCTO SET DESCRIPCION=D_INS,PRECIO=P_INS, STOCK=S_INS, 
		STOCK_CRITICO=SC_INS, ESTADO=1 
		WHERE CODIGO=C_INS;
	ELSIF OPCION=3 THEN
		UPDATE PRODUCTO SET ESTADO=0 WHERE CODIGO=C_INS;
	ELSIF OPCION=4 THEN
		UPDATE PRODUCTO SET ESTADO=1 WHERE CODIGO=C_INS;
	END IF;
	END;
/